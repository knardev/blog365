...
export async function GET(_request: Request) {
  ...
  console.log("[ROUTE] Starting to push keyword scraping tasks...");

  const result = await pushKeywordScrapingTasks();

  if (!result.success) {
    console.error(
      "[ERROR] Failed to push keyword scraping tasks:",
      result.error,
    );
    return new Response(
  ...
  console.log(
    `[SUCCESS] Successfully pushed ${result.count} keyword scraping tasks to the queue.`,
  );
